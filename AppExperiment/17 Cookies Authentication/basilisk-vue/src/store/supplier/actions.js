...
export default{
    ...
    async refreshGrid(){
        const {page, company, contact, jobTitle} = this;
        Swal.fire({
            title: 'Loading...',
        ...
        const response = await axios.get(`/supplier?page=${page}&company=${company}&contact=${contact}&jobTitle=${jobTitle}`);
        Swal.close();
    ...
    async upsert(parameters){
        ...
        const response = await axios[method](`/supplier`, payload).then(response => {
            Swal.close();
            this.refreshGrid();
            return response;
        });
