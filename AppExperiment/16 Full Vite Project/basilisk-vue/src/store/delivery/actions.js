...
export default{
    ...
    async refreshGrid(){
        ...
        const response = await axios.get(`/delivery?page=${page}&company=${company}`);
        Swal.close();
    ...
    async upsert(parameters){
        ...
        const response = await axios[method](`/delivery`, payload).then(response => {
            Swal.close();
            this.refreshGrid();
            return response;
        });
